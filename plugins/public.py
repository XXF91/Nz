...
async def run(bot, message):
    ...
    if not _bot:
      ...
      if not _bot:
          return await message.reply("<code>لم تقم بإضافة أي روبوت. يرجى إضافة روبوت باستخدام /settings !</code>")
    channels = await db.get_user_channels(user_id)
    if not channels:
       return await message.reply_text("يرجى تعيين القناة في /settings قبل إعادة التوجيه")
    if len(channels) > 1:
    ...
    if fromid.text and not fromid.forward_date:
        ...
        if last_msg_id == None:
           return await message.reply_text("**قد تكون هذه رسالة معاد توجيهها من مجموعة وأرسلها مسؤول مجهول. بدلاً من ذلك، يرجى إرسال رابط الرسالة الأخيرة من المجموعة**")
    else:
